...
class MyWishListController extends GetxController {
  ...
  var wishListModel = MyWishListModel(message: '', products: {}).obs;
        ...
        wishListModel.value.products.values.forEach((element) {
          ...
          element.forEach((element2) {
            if (element2.type == ProductType.GIFT_CARD) {
              ...
              wishListProducts.add(WishListLocal(
                  id: element2.product.id, productType: ProductType.PRODUCT));
            }
          });
        });
        wishListCount.value = count;
      } else {
        wishListModel.value = MyWishListModel(message: '', products: {});
      }
      return wishlist;
    } catch (e) {
      // throw e.toString();
